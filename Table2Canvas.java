...
class Table2Canvas extends ScrollableCanvas implements Selectable
{
   ...
   */
    public void paint(Graphics g)
    {
        ...
        boolean selectedRowIsVisible = (startRowN <= selectedRowN && selectedRowN <= endRowN);
        boolean selectedColIsVisible = (startColN <= selectedColN && selectedColN <= endColN);
        ...
        if (selectedRowIsVisible)
        {
            g.fillRect(0, rowStarts[selectedRowN] + headHeight - y, headWidth,
                rowStarts[selectedRowN + 1] - rowStarts[selectedRowN] - ROW_SEP);
        }
        if (selectedColIsVisible)
        {
            g.fillRect(colStarts[selectedColN] + headWidth - x, 0,
                colStarts[selectedColN + 1] - colStarts[selectedColN] - COL_SEP,
                headHeight);
        }
        ...
        g.setColor(contentsBGColor);
        g.fillRect(0, 0, headWidth, headHeight);
    }
   ...
   */
    private void updateParams()
    {
        ...
        headHeight += ROW_SEP;
        headHeight = (int)Math.ceil((double)(headHeight + ROW_DIV - 1) / ROW_DIV * ROW_DIV);
        ...
        for (int i = 0; i < nRows; i++)
        {
            ...
            for (int j = 0; j < nCols; j++)
            {
                Object col = table.getCol(j);
                Object val = table.get(row, col);
                int valHeight = (val == null) ? 0 : stringHeight(table.get(row, col).toString(), m);
                if (valHeight > hRow)
                    hRow = valHeight;
            }
            hRow += ROW_SEP;
        ...
        for (int i = 0; i < nRows; i++)
        {
            int w = stringWidth(table.getRow(i).toString(), m);
            if (w > headWidth)
                headWidth = w;
        }
        headWidth += COL_SEP;
        headWidth = (int)Math.ceil((double)(headWidth + COL_DIV - 1) / COL_DIV * COL_DIV);
   ...
   */
    ...
    static private final Color headSelBGColor = new Color(180, 180, 180); //dark grey.

    //background color to use for selected row/col.
    static private final Color contentsSelBGColor = new Color(230, 230, 230); //lighter grey than headBGColor.

    //background color for table entry at intersection of selected row/col.
    static private final Color contentsSel2BGColor = new Color(170, 170, 170); //darker grey than headSelBGColor.
