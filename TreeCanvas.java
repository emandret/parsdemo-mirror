...
class TreeCanvas extends ScrollableCanvas implements Selectable
{
    ...
    private void updateScrollbars(int x, int y, int width, int height)
    {
        ...
        hScroll.setValues(x1, s.width, 0, xMax + SLOP);
        int yMax = s.height > height ? s.height : height;
        int y1 = (y <= height) ? y : height;
        vScroll.setValues(y1, s.height, 0, yMax + SLOP);
    }
    ...
    Image getHintImage()
    {
        ...
        Dimension s = size();
        if (hintMsg != null && (hintBuf == null || hintBufDim.width != s.width || hintBufDim.height != m.getHeight() + 2 * HINT_Y_INSET))
        {
            int h = m.getHeight();
    ...
    private static final Color RECT_SHADOW_COLOR = new Color(RECT_GRAY, RECT_GRAY, RECT_GRAY);
    private static final Color LINE_SHADOW_COLOR = new Color(LINE_GRAY, LINE_GRAY, LINE_GRAY);
    ...
    static public void main(String args[])
    {
        ...
        t00.addKid(new OffsetTree(new ParseNode("F", true),
                       new OffsetTree(new ParseNode("G", true))),
            1);
        ...
        OffsetTree x = new OffsetTree(new ParseNode("stmts", false),
